...
class MDP(object):
    ...
    def qvalueIteration(self, iterations=200, threshold=0.000000001, debug=False):
        previousMatrix = self._initial_v
        previousQMatrix = [[0 for _ in self._actions] for _ in self._states]
        returnQMatrix = [[0 for _ in self._actions] for _ in self._states]
            ...
            for s in range(len(self._states)):
                ...
                for a in range(len(self._actions)):
                    ...
                    for sp, prob in possibleOutcomes:
                        ...
                        for act in previousQMatrix[sp]:
                            max_action_value = max(act, max_action_value)
                        actionValue += prob * (self._reward(s, a, sp) + self._discount * max_action_value)
                    returnQMatrix[s][a] = actionValue
                    maxValue = max(maxValue, actionValue)
                returnMatrix[s] = maxValue
                delta = max(delta, abs(previousMatrix[s] - returnMatrix[s]))
            previousMatrix = returnMatrix
            previousQMatrix = returnQMatrix
            if debug:
                print("Iteration # {iter}".format(iter=i))
                print(previousQMatrix)
            if delta < threshold:
                break
